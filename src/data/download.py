...
def download_from_kaggle(
    ...
    try:
        ...
        print(
            "Please download manually from: https://www.kaggle.com/datasets/bhavikjikadara/dog-and-cat-classification-dataset"
        )
        raise
...
def organize_dataset(raw_dir: str = "data/raw", output_dir: str = "data/processed") -> dict:
    ...
    if not training_cats:
        training_cats = list(
            (raw_path / "dataset" / "training_set" / "training_set" / "cats").glob("*.jpg")
        )
        training_dogs = list(
            (raw_path / "dataset" / "training_set" / "training_set" / "dogs").glob("*.jpg")
        )
    ...
    def split_and_copy(
        files: list, class_name: str, train_ratio: float = 0.8, val_ratio: float = 0.1
    ):
    ...
    for split in splits:
        ...
        print(
            f"  {split}: {total} images (cats: {counts[split]['cats']}, dogs: {counts[split]['dogs']})"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-samples", type=int, default=100, help="Number of samples for sample dataset"
    )
